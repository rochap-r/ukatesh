...
@section('title','Edition d\'image')
    ...
    @section('content')
                ...
                <div class="row g-2 align-items-center">
                    <div class="col">
                        <h2 class="page-title text-uppercase text-success">
                            Edition d'image de la galérie {{ $galery->type->name }}
                        </h2>
                    </div>
                    <!-- Page title actions -->
        ...
        <div class="container py-5">
            <div class="row justify-content-center">
                <div class="col-md-6">
                  <form  method="POST" action="{{ route('admin.gallery.update',$galery->id) }}" id="editGalery" enctype="multipart/form-data">
                    @csrf
                    <div class="image_holder mb-3 text-center">
                        <img src="" alt="" class="img-thumbnail" id="image-previewer" data-ijabo-default-img="
                        {{ $galery->name? asset('storage/galeries/thumbnails/thumb_'.$galery->name) : '' }}
                        ">
                    </div>
                      ...
                      <label for="categorySelect">Type d'image:</label>
                        <select class="form-control" id="categorySelect" name="type" id="form-type">
                            ...
                            @foreach(\App\Models\Type::all() as $type)
                                <option {{ $type->id==$galery->type->id ? 'selected' : ''}} value="{{ $type->id }}">{{ $type->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group h2 mb-3">
                      ...
                      <input type="text" class="form-control" id="titleInput" name="title" value="{{ $galery->title }}">
    ...
    @push('script')
        ...
        <script type="text/javascript">
            ...
            $('form#editGalery').on('submit', function(e) {
                e.preventDefault();
                toastr.remove();
                var form=this;
